...
export class CrudService implements OnDestroy {
   ...
   */
  getList(route: string, correo?: string) {
    route = correo === undefined ? route : route + `?correo=${correo}`
    ...
    return this.http.patch(`${this.apiUrl}/users/${id}/status`, { isActive }).pipe(
      tap(() => {
        this.messageService.add({
          ...
          detail: `El usuario ha sido ${isActive ? 'activado' : 'desactivado'} con éxito.`,
        });
      }),
      catchError((error) => {
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: error.error.message || 'Algo salió mal. Inténtalo de nuevo.',
        });
        return throwError(error);
      })
    );
